...
`Cargo.nix` files generated by crate2nix contain a shell expression that
provides an environment with the combined build inputs and environment
variables of all workspace crates. This is a suitable shell for development
with cargo directly.

This shell can be invoked as follows:

```bash
nix-shell Cargo.nix -A shell
```

You can also extend an existing `mkShell` expression with the shell expression
from `crate2nix` using `inputsFrom`:

```nix
let
  pkgs = import <nixpkgs> {};
  crateShell = pkgs.callPackage ./Cargo.nix {};
in pkgs.mkShell {
  inputsFrom = [ crateShell ];
  buildInputs = [
    pkgs.gnumake
  ];
}
...
This will create a shell containing everything from the `crate2nix` generated
shell plus the package `gnumake`.
