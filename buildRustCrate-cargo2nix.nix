{ lib, stdenv, defaultCrateOverrides, callPackage, fetchCrate, rustc, rust, cargo, jq, rustBuilder }:
...
let
  ...
  overridable =
    ...
    let
      ...
      dependenciesNameValue = dependencies:
        map
          (dep:
            let
      ...
      madeCrate = mkRustCrate {
        ...
        profile = null;
        buildDependencies = builtins.listToAttrs (dependenciesNameValue buildDependencies);
        dependencies = builtins.listToAttrs (dependenciesNameValue dependencies);
        features = builtins.filter (feature: (builtins.match ''.*/.*'' feature) == null) features;
        rustcflags = [
          "--cap-lints"
          "warn"
        ];
...
in
   ...
   */
crate_: lib.makeOverridable (overridable crate_) {
  rust = rustc;
  release = crate_.release or true;
  verbose = crate_.verbose or true;
  extraRustcOpts = crate_.extraRustcOpts or [ ];
  features = crate_.features or [ ];
  buildInputs = crate_.buildInputs or [ ];
